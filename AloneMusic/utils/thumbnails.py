...
async def get_thumb(videoid):
    ...
    try:
        ...
        g_left, g_top, g_right, g_bottom = glass_box
        glass_crop = bg.crop(glass_box).filter(ImageFilter.GaussianBlur(12))
        glass_tint = Image.new("RGBA", glass_crop.size, (255, 255, 255, 34))
        glass_panel = Image.alpha_composite(glass_crop, glass_tint)

        glass_mask = Image.new("L", glass_panel.size, 0)
        glass_draw = ImageDraw.Draw(glass_mask)
        glass_draw.rounded_rectangle(
            [0, 0, glass_panel.width, glass_panel.height],
            radius=48,
            fill=255,
        )

        glass_layer = Image.new("RGBA", bg.size, (0, 0, 0, 0))
        glass_layer.paste(glass_panel, (g_left, g_top), glass_mask)
        glass_border = ImageDraw.Draw(glass_layer)
        glass_border.rounded_rectangle(
        ...
        depth_tint = Image.new("RGBA", bg.size, (12, 20, 35, 10))
        ...
        blurred_panel = cropped.filter(ImageFilter.GaussianBlur(6))
        white_overlay = Image.new("RGBA", blurred_panel.size, (255, 255, 255, 14))
